...
# Check with Pieter to improve this part:
localities <- lapply(sites_samples, function(x) {
  df <- data.frame(area_name = NA)
  df$area_name <- x$area_name
  df$area_locality <- x$area_locality
  df$station <- x$station
  df$lon <- x$area_longitude
  df$lat <- x$area_latitude
  df
})
localities <- dplyr::bind_rows(localities)
```
