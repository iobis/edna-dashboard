...
get_temperature_species <- function(force = FALSE) {
    


    if (file.exists(file.path("data", "species_sites_risk_full.parquet"))) {
        if (!force) {
            cat("Species temperature file already available.\n")
    ...
    occ <- occ %>%
        ...
        select(scientificName, higherGeography, phylum, class, order) %>%
        group_by(scientificName, higherGeography, phylum, class, order) %>%
    ...
    write_parquet(sites_values, file.path("data", "sites_sst_all.parquet"))
    ...
    sites_species_risk <- species_site_temp %>%
        ...
        mutate(across(6:length(.), ~round(.x, 2)))
    ...
    sites_species_risk <- sites_species_risk %>%
    ...
    sites_species_risk <- sites_species_risk %>% left_join(redlist)
    ...
    sites_species_risk_f <- left_join(sites_species_risk, funcinfo[,c("scientificName", "habitat")])
    ...
    deep_sp <- sites_species_risk_f %>%
        filter(!habitat %in% c("Shallow", "Not available")) %>%
        select(scientificName, phylum, class, higherGeography, contains("depthmean"), sp_limit = limit_dmean,
            group, category, habitat)
    ...
    fs::dir_delete(datafolder)

    cat("Species temperature dataset generated.\n")
