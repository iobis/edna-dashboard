...
htmltools::tags$div(
  ...
  leafletOutput("mainMap", height = 400), id = "top-selector-parent"
)
```
...
htmltools::span(
  htmltools::span(
    "World Heritage Site", htmltools::HTML("&nbsp;"), textOutput("whs_front_code"),
    style = "display: flex; font-weight: bold; font-color: #cccccc;"
  ), htmltools::HTML("&nbsp;"), "|", htmltools::HTML("&nbsp;"), htmlOutput("whs_front_website"), style = "display: flex;"
)
```
...
// Select the div with the class you specified
let div = document.querySelector('.bslib-grid-item.html-fill-item.tab-content.bslib-grid');

// Check if the div exists to avoid errors
if (div) {
    // Remove the grid-template-rows style from the inline style
