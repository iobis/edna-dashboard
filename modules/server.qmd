...
mdebug <- function(status, show = FALSE) {message(status)}
...
output$species_gallery <- renderUI({
    req(!is.null(species_list()$scientificName) || length(species_list()$scientificName) > 0)
    ...
    if (length(species_list()$scientificName) > sg_limit()) {
      last_name <- species_list()$scientificName[sg_limit()]
      last_name <- paste0("link_", gsub(" ", "_", last_name))
...
observe({
  if (length(species_list()$scientificName) <= sg_limit()) {
    ...
    wb$show()
    on.exit(wb$hide())
    gallery_b <- generate_gallery(species_list()$scientificName[sg_limit():length(species_list()$scientificName)], "data/proc_images.txt", max_image_height = "250px",max_image_width = "200px", image_gap = "0.1em")
