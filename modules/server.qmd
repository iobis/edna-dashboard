...
output$tax_bar_plot <- renderUI({
  ...
  itax <- make_image_taxonomy(occurrence_ds, input$higherGeography, input$taxonLevelDiversity, input$plotType)
...
output$samples_table <- renderReactable({
  reactable(
    ...
    columns = list(
      ...
      volume = colDef(name = "Volume (mL)", width = 120),
      concentration = colDef(name = "Concentration (ng/\u00b5l)"),
      date = colDef(
        name = "Date",
        width = 140,
        cell = function(value) {
