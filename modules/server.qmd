...
observe({
  if (!is.null(input$clicked_image_id)) {
    ...
    output$species_info <- reactable::renderReactable({reactable::reactable(species_info, defaultPageSize = 4, columns = list(
...
output$tax_bar_plot <- renderPlot({
 #taxbarplot() 
 make_image_taxonomy(occurrence, input$higherGeography, input$taxonLevel, input$plotType)}, height=400
)
