...
# Update first page options when not at Home
observe({
  if (!is.null(input$activeTab)) {
    if (input$activeTab != "Home" && input$higherGeography == ""){
      updateSelectInput(session, "higherGeography",
      choices = sites_names, selected = sites_names[1])
    }
  }
}) %>%
  bindEvent(input$activeTab)
